...
#                    metrics=['accuracy'])
    json_file = open(os.path.join("models", "CNN_json_model.json"), 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model.load_weights(os.path.join("models", "CNN_json_weights.h5"))
    # loaded_model = load_model("pickles//model_final.h5")
    model4_pred = loaded_model.predict_classes(data_2dfeatures)[0]
